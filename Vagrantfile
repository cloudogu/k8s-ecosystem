...
fqdn = "k3ces.localhost"
kube_ctx_name= "k3ces.localhost"
  ...
  config.vm.define "main", primary: true do |main|
    ...
    main.trigger.before :up do |trigger|
      if certificate_type == "mkcert"
        ...
        # create certificates
        if ! File.file?(".vagrant/certs/k3ces.localhost.crt") || !File.file?(".vagrant/certs/k3ces.localhost.key")
          `mkdir -p .vagrant/certs`
          `mkcert -cert-file .vagrant/certs/k3ces.localhost.crt -key-file .vagrant/certs/k3ces.localhost.key #{fqdn} #{main_k3s_ip_address}`
        end

      else
        # remove geneated certificate files
        if File.file?(".vagrant/certs/k3ces.localhost.crt")
          File.delete(".vagrant/certs/k3ces.localhost.crt")
        end
        if File.file?(".vagrant/certs/k3ces.localhost.key")
          File.delete(".vagrant/certs/k3ces.localhost.key")
        end
      end
    end
